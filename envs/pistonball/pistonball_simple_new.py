...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(
        ...
        assert (
            self.piston_width * 2 == self.wall_width
            ...
            zip(
                ...
                [
                    gymnasium.spaces.Box(
                        low=-float('inf'),
                        high=float('inf'),
                        shape=(l_n_obs_neighbors + self.n_obs_neighbors + 1 + 5, 1),
                        dtype=np.float32,
                    )
                ]
                * self.n_pistons,
        ...
        self.frames = 0
        self.ball_stay_count = 0
        self.ball_stay_limit = 3
        self.init_ball_stay_limit = 10
        self.left_neighbor = l_n_obs_neighbors
        self.pos_y_size = self.n_pistons + self.left_neighbor + self.n_obs_neighbors
        self.piston_pos_y = np.full(self.pos_y_size, self.mid_piston_y)  # y position of each piston
        self.ep_steps = 0
    ...
    def observe(self, agent):
        all_piston_pos = (self.piston_pos_y - self.mid_piston_y) / self.piston_y_half_range
        agent_id = self.agent_name_mapping[agent]
        observation_piston = all_piston_pos[agent_id:self.left_neighbor + agent_id + self.n_obs_neighbors + 1]

        ball_left_x = self.ball.position[0] - self.ball_radius
        ball_right_x = self.ball.position[0] + self.ball_radius
        piston_right_x = self.wall_width + self.piston_width * (agent_id + 1 + self.n_obs_neighbors)
        piston_left_x = self.wall_width + self.piston_width * (agent_id - self.left_neighbor)
        ball_above = piston_right_x > ball_left_x and piston_left_x < ball_right_x

        if ball_above:
            piston = self.pistonList[agent_id]
            ball_bottom = self.ball.position.y + self.ball_radius
            ball_right = self.ball.position.x + self.ball_radius
            observation_ball = np.array(
                [(ball_right - piston.position[0]) / (max(1, self.n_obs_neighbors) * self.piston_width),
                 (ball_bottom - self.mid_piston_y) / self.piston_y_half_range,
                 self.ball.angular_velocity / 8, self.ball.velocity.x / 15,
                 self.ball.velocity.y / 8])
        else:
            observation_ball = np.array([3 if piston_right_x < ball_left_x else -3, -3, 0, 0, 0])
        observation = np.concatenate((observation_piston, observation_ball))
    ...
    def reset(self, seed=None, return_info=False, options=None):
        ...
        for i in range(self.n_pistons):
            ...
            self.piston_pos_y[i + self.left_neighbor] = piston.position[1]
        ...
        self.last_ball_neighbors = self.get_nearby_pistons()
        ...
        self.infos = dict(
            zip(self.agents, [{'pos_y': (self.maximum_piston_y - self.piston_pos_y[i + self.left_neighbor]) / self.pixels_per_position
    ...
    def get_nearby_pistons(self):
        ...
        for i in range(self.n_pistons):
            if ball_pos < self.pistonList[i].position.x + self.piston_width:
        ...
        if closest_piston_index < self.n_pistons - 2:
            nearby_pistons.append(closest_piston_index + 2)
        if closest_piston_index < self.n_pistons - 3:
            if int(self.ball.position[0] + self.ball_radius) > self.pistonList[closest_piston_index + 3].position.x:
                nearby_pistons.append(closest_piston_index + 3)
    ...
    def step(self, action):
        ...
        agent_id = self.agent_name_mapping[agent]
        if self.continuous:
            self.move_piston(self.pistonList[agent_id], action)
        else:
            self.move_piston(
                self.pistonList[agent_id], action - 1
            )

        self.piston_pos_y[agent_id + self.left_neighbor] = self.pistonList[agent_id].position[1]

        self.space.step(self.dt)
        if self._agent_selector.is_last():
            current_ball_neighbors = self.get_nearby_pistons()
            # self.space.step(self.dt)
            combined_neighbor_list = current_ball_neighbors + self.last_ball_neighbors
            self.ep_steps += 1
            if sorted(current_ball_neighbors) == sorted(self.last_ball_neighbors):
                self.ball_stay_count += 1
            else:
                self.ball_stay_count = 0
            local_pistons_to_reward = [neighbor for neighbor in range(min(combined_neighbor_list),
                                                                      max(combined_neighbor_list) + 1)]
            self.last_ball_neighbors = current_ball_neighbors
            ...
            if self.ball_stay_count >= self.ball_stay_limit and self.ep_steps >= self.init_ball_stay_limit \
                    and not self.terminate:
                local_reward += self.time_penalty
            # Opposite order due to moving right to left
            total_reward = [0 for _ in range(self.n_pistons)]
